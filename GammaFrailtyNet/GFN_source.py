...
def gf_train_model(
    ...
    for epoch in range(pretrain):
        train_loss = gf_train_one_epoch(mean_model, train_batch, d_sum_train, cluster_size_train, lam, pretrain_optimizer, disp=None)
    ...
    for epoch in range(max_epochs):
        if epoch%disp_update == 0:
            if disp_method == 'Newton':
                train_loss = gf_train_one_epoch(mean_model, train_batch, d_sum_train, cluster_size_train, lam, optimizer, disp=None)                
